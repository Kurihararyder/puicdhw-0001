...
#include <stdlib.h>
#include <math.h>
#include <ctype.h> // 必須加入這個，用於 isspace()
...
// 輔助函式：跳過 PGM 檔頭的註解 (#) 和空白
void skipComments(FILE *fp) {
    int ch;
    char line[1024];

    // 1. 跳過所有空白字符 (空格、換行、Tab)
    ...
    if (ch == '#') {
        ...
        // 遞迴呼叫：讀完一行註解後，可能下一行還是註解，或者還有空白，所以繼續跳
        skipComments(fp);
    } else {
        // 3. 如果不是註解，把剛剛讀到的那個非空白字元「退回」串流中
        // 這樣接下來的 fscanf 才能讀到正確的數字
        fseek(fp, -1, SEEK_CUR);
    }
}
...
void readPGM(const char *filename, Image *img) {
    ...
    // --- 關鍵修正：讀取寬高前，先跳過可能存在的註解 ---
    skipComments(fp);
    fscanf(fp, "%d %d", &img->width, &img->height);

    // --- 關鍵修正：讀取最大灰階值前，也要跳過註解 ---
    skipComments(fp);
    int max_val;
    fscanf(fp, "%d", &max_val);
    ...
    if (img->width <= 0 || img->height <= 0) {
        printf("錯誤：讀取到的影像尺寸無效 (%dx%d)。可能是檔案格式問題。\n", img->width, img->height);
        fclose(fp);
        exit(1);
    }
...
int main() {
    // 使用 static 可以避免 stack overflow (當圖片很大時)
